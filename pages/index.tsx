...
import { usePersistantState } from '../components/lib/persistant-state'
import { ThemeContext, ThemeContextProps } from '../components/lib/themecontext'
import { openNodeInEmacs } from '../components/lib/webSocketFunctions'
import { drawLabels } from '../components/Graph/drawLabels'
import { VariablesContext } from '../components/lib/variablesContext'
import { findNthNeighbors } from '../components/lib/findNthNeighbour'
import { getThemeColor } from '../components/lib/getThemeColor'
import { normalizeLinkEnds } from '../components/lib/normalizeLinkEnds'
import { nodeSize } from '../components/lib/nodeSize'
import { getNodeColor } from '../components/lib/getNodeColor'
import { isLinkRelatedToNode } from '../components/lib/isLinkRelatedToNode'
import { getLinkColor } from '../components/lib/getLinkColor'
import { Sidebar } from '../components/Sidebar'

const d3promise = import('d3-force-3d')

// Use dynamic imports to avoid SSR issues and prevent AFRAME from being loaded unnecessarily
const ForceGraph2D = dynamic(
  () => import('react-force-graph').then((mod) => mod.ForceGraph2D),
  { ssr: false, loading: () => null }
) as unknown as typeof TForceGraph2D

const ForceGraph3D = dynamic(
  () => import('react-force-graph').then((mod) => mod.ForceGraph3D),
  { ssr: false, loading: () => null }
) as unknown as typeof TForceGraph3D
...
export function GraphPage() {
  ...
  const [previewNode, setPreviewNode] = useState<NodeObject | null>(null)
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const [canUndo, setCanUndo] = useState(false)
  const [canRedo, setCanRedo] = useState(false)
  const resetPreviewNode = () => setPreviewNode(null)
    ...
    <VariablesContext.Provider value={{ ...emacsVariables }}>
      ...
      >
        ...
        <Sidebar
          ...
          canUndo={canUndo}
          canRedo={canRedo}
          resetPreviewNode={resetPreviewNode}
